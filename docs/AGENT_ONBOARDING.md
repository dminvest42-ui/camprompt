...
## 4. Шесть граблей, на которых уже спотыкались
...
6. **AVFoundation на macOS — не iOS.** Удобные API для записи (`supportedOutputSettingsKeys`, `availableVideoCodecTypes`) есть только на iOS, а неподходящая настройка роняет приложение исключением, которое Swift не ловит. Отсюда `CPTryObjC` и откат по ступеням. [D-020](DECISIONS.md#d-020-качество-записи-hevc-с-потолком-битрейта-и-ступенчатый-откат)
